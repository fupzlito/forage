...
class TestMCPAndOpenAIEndpoints(unittest.TestCase):
    ...
    def test_mcp_tools_list(self):
        # youtube_search is registered only when an API key is configured.
        config_with_key = dataclasses.replace(
            config, youtube=dataclasses.replace(config.youtube, api_key="TEST_KEY")
        )
        with patch.object(app.state, "config", config_with_key):
            payload = {
                "jsonrpc": "2.0",
                "id": 2,
                "method": "tools/list",
    ...
    def test_mcp_messages_queue_routing(self):
        """POST /mcp/messages?session_id=... with an active SSE session routes the RPC
        result into the per-session queue (covers ``if sid in _sse_sessions``).

        The whole flow runs in ONE event loop: opening the session and calling the
        handler in separate ``asyncio.run`` loops would trip Python's
        per-loop async-generator shutdown, which closes the SSE generator and
        evicts the session before the handler runs."""
        import asyncio
        from app.mcp import mcp_messages, mcp_sse, _sse_sessions
        ...
        async def _flow():
            ...
            sid = first.split("session_id=")[1].split()[0]
            assert sid in _sse_sessions

            post_req = MagicMock()
            post_req.app.state.config = None
            post_req.app.state.browser_pool = None
        ...
        self.assertEqual(json.loads(resp2.body), {"status": "accepted"})
        self.assertEqual(item["id"], 9)
        self.assertIn("tools", item["result"])
        names = [t["name"] for t in item["result"]["tools"]]
        self.assertIn("web_search", names)

    def test_mcp_post_queue_routing(self):
        """POST /mcp with an explicit session_id takes the same queue branch
        (``mcp_post``): acknowledged with 200 and the result lands in the
        per-session queue."""
        import asyncio
        from app.mcp import mcp_post, mcp_sse, _sse_sessions
        ...
        self.assertIn("serverInfo", item["result"])
