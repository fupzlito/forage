...
The tool is active only when a YouTube Data API v3 key is configured
(`youtube.api_key` / `FORAGE_YOUTUBE_API_KEY`); there is no keyless fallback.
"""
...
from app.searxng import format_citation
...
def resolve_handle_to_channel_id(
    ...
    # Pass the key via header so it never appears in the request URL (and thus
    # never leaks into httpx exception strings / error responses).
    ...
    try:
        ...
        resp_user = client.get(url, params=params_user, headers=headers)
...
def search_youtube_direct(
    ...
    if clean_channel:
        ...
        if extracted_cid:
            ...
            if cid is None:
                return {
                    "success": False,
                    "error": f"Could not resolve channel '@{extracted_handle}'",
                    "results": [],
                }
        else:
            return {
                "success": False,
                "error": f"Could not parse channel '{clean_channel}' as a channel ID or @handle",
                "results": [],
            }
    ...
    try:
        resp = client.get(f"{_YOUTUBE_API_BASE}/search", params=api_params, headers=api_headers)
...
def search_youtube(
    ...
    """Execute a YouTube search via the direct YouTube Data API.

    Requires a configured API key (`youtube.api_key` / `FORAGE_YOUTUBE_API_KEY`);
    there is no keyless fallback.
    """
    if not config.youtube.api_key:
        return {
            "success": False,
            "error": "YouTube search requires FORAGE_YOUTUBE_API_KEY to be configured.",
            "results": [],
        }
