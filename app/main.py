...
from .extract import clamp_output
from .mcp import _extract_one, mcp_router
...
class SearchRequest(BaseModel):
    ...
    engines: Optional[List[str]] = Field(
        default=None,
        description=(
            "Optional list of search engines to query. Engine name aliases "
            "(e.g. 'ddg', 'google_search') will be auto-mapped or filtered gracefully."
        ),
    )
...
async def extract(
    ...
    async def _extract_one_full(url: str, pos: int) -> Dict[str, Any]:
        """Delegate to the shared mcp pipeline (fetch -> clamp -> error-wrap);
        keeps the REST error shape ({'url', 'error'} without error_type)."""
        try:
    ...
    if cache_enabled:
        ...
        if cached is not None and isinstance(cached, dict):
            truncated_data = [clamp_output(r, req.max_chars) for r in cached.get("data", [])]
