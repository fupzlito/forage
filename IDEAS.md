...
4. **Fallback**: if the probe throws/times out (or succeeds but returns no engines), `get_live_available_engines` falls back to the built-in `DEFAULT_AVAILABLE_ENGINES` tuple (`searxng.py:149-152`): 12 names (`google`, `qwant`, `qwant news`, `brave`, `bing`, `startpage`, `duckduckgo`, `wikipedia`, `github`, `searxng`, `yahoo`, `wikidata`), a static catalog NOT validated against your instance and with no SearXNG-specific engines (e.g. `youtube`, `reddit`). The static and live catalogs can disagree about which engines are actually present.

So during (or after) a slow SearXNG cold start, Forage *tells the model* a static catalog that may name engines your instance never has (the real 12-name fallback: `google`, `qwant`, `qwant news`, `brave`, `bing`, `startpage`, `duckduckgo`, `wikipedia`, `github`, `searxng`, `yahoo`, `wikidata`; a fixed catalog, NOT validated against your instance, and it omits SearXNG-specific engines like `youtube`/`reddit`); every call then yields the "ignored unknown engine(s)" warning + default-engine fallback. Both the probe-failure path and the empty-result fallback are logged at `logger.warning` (probe failure: `searxng.py:198`; empty result: `searxng.py:234`).
...
4. **IDEA 2: liveness telemetry** (done: `2e1d7e3` + `d203f29`): both the probe-failure path (`searxng.py:198`) and the no-engine fallback (`searxng.py:234`) now log at `logger.warning`; a module-level `_engine_lock` (double-checked) prevents concurrent TTL-expiry stampede on the cache.
